...
{
    ESP_LOGD(TAG, "Display hor size: %d, ver size: %d", LV_HOR_RES_MAX, LV_VER_RES_MAX);
    ESP_LOGD(TAG, "Display buffer size: %d", DISP_BUF_SIZE);

#if defined (CONFIG_LV_TFT_DISPLAY_CONTROLLER_FT81X)
    ESP_LOGD(TAG, "Initializing SPI master for FT81X");
    ...
    ESP_LOGD(TAG, "Initializing shared SPI master");
    ...
    ESP_LOGD(TAG, "Initializing shared I2C master");
...
/* Display controller initialization */
    ...
    ESP_LOGD(TAG, "Initializing SPI master for display");
    ...
    ESP_LOGD(TAG, "Initializing I2C master for display");
...
/* Touch controller initialization */
        ...
        ESP_LOGD(TAG, "Initializing SPI master for touch");
        ...
        ESP_LOGD(TAG, "Initializing I2C master for touch");
...
bool lvgl_i2c_driver_init(int port, int sda_pin, int scl_pin, int speed_hz)
{
    esp_err_t err;
    // return true;
    ESP_LOGD(TAG, "Initializing I2C master port %d...", port);
    ESP_LOGD(TAG, "SDA pin: %d, SCL pin: %d, Speed: %d (Hz)",
    ...
    ESP_LOGD(TAG, "Setting I2C master configuration...");
    err = i2c_param_config(port, &conf);
    assert(ESP_OK == err);

    ESP_LOGD(TAG, "Installing I2C master driver...");
    ...
    int quadwp_pin, int quadhd_pin)
{
    ...
    ESP_LOGD(TAG, "Configuring SPI host %s (%d)", spi_names[host], host);
