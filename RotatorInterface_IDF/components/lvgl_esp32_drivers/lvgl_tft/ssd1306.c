...
#define OLED_HEIGHT                         CONFIG_LV_TFT_DISPLAY_CONTROLLER_SSD1306_VER_RES
...
void ssd1306_init(void)
{
    ...
    uint8_t conf[] = {
        ...
        OLED_CMD_SET_CHARGE_PUMP,
        0x14, // charge pump ON
        // display scan orientation, remap means from top to bottom, from left to right
        0xA1, // SEG remap
        0xC8, // COM remap, note 32px OLED's row index begins from 32, all the way to 63, the first page number will be 4
        
        OLED_CMD_SET_COM_PIN_MAP,
        0x02, // height = 32: 0x02 if height = 64: 0x12
        OLED_CMD_SET_CONTRAST,
        0x8F,
        OLED_CMD_DISPLAY_INVERTED, // illuminate texts, darken the background
        0xFF,
        OLED_CMD_DISPLAY_ON
    };
#endif
...
#if (OLED_HEIGHT == 32U)
    row1 += 4; // for 32px OLED, row index begins from 32, all the way to 64. need to offset by 4 pages
    row2 += 4;
#endif
    uint8_t conf[] = {
        OLED_CONTROL_BYTE_CMD_STREAM,
...
static uint8_t send_pixels(lv_disp_drv_t *disp_drv, void *color_buffer, size_t buffer_len)
{
    ...
    /* Send queued commands */
    err = i2c_master_cmd_begin(DISP_I2C_PORT, cmd, pdMS_TO_TICKS(10));
    i2c_cmd_link_delete(cmd);
